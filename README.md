...
## Deep nesting / Layered architecture

A common question is: _"Do I have to call `get::<HttpClient>()` in `main` and thread it through every layer?"_

No. `Depends` generates `Default` for structs whose `#[dep]` fields resolve automatically. When you nest those structs, the `Default` chain resolves every dependency for you — no matter how deep.

### Automatic resolution through layers
...
#[derive(Depends)]
struct Fetcher {
    api: Api, // plain field — Api's generated Default resolves Http automatically
}

#[derive(Depends)]
struct App {
    fetcher: Fetcher, // plain field — chains through Fetcher → Api → Http
}

let app = App::from_deps();
# // Http was resolved 3 layers deep without touching it in main.
```

`Http` was resolved three layers deep without a single `get::<Http>()` call in `main`.

### Explicit fields with `#[arg]`

Some fields are not dependencies and have no sensible `Default` — configuration, for example. Mark those with `#[arg]` and they become parameters of `from_deps()`:

```rust
use clients::{Depends, client};
...
// Only Config is passed explicitly — Http resolves automatically 3 layers deep.
...
When `#[arg]` fields are present, `Depends` does **not** generate `impl Default` (the struct cannot be defaulted without those values). It generates only `from_deps(...)` with the `#[arg]` fields as parameters in field-declaration order.

Field attribute summary:

| Attribute | Initializer                | Purpose                             |
| --------- | -------------------------- | ----------------------------------- |
| `#[dep]`  | `get::<T>()`               | Resolved from the dependency system |
| `#[arg]`  | parameter of `from_deps()` | Caller-supplied, no default         |
| _(none)_  | `Default::default()`       | Plain field with a sensible default |

Rules:

- `#[dep]` and `#[arg]` are **mutually exclusive** on a single field
- Multiple `#[arg]` fields become multiple parameters, ordered as declared

### Comparison with swift-dependencies

This pattern is the Rust equivalent of the TCA + swift-dependencies experience where the `Store` is created and dependencies resolve implicitly:

```swift
// Swift — dependencies resolve automatically inside reducers
App.View(store: Store(initialState: App.State()) { App() })
```

```rust
// Rust — dependencies resolve automatically through the Depends chain
let app = App::from_deps(Config { /* ... */ });
```

Like swift-dependencies, you can optionally override specific clients for tests:

```rust
use clients::test_deps;

test_deps! {
    http.get_text => |url| Ok(format!("[test stub for {url}]")),
}
...
// Http is now stubbed 3 layers deep — no manual injection needed.
```

See [examples/layered_app.rs](./examples/layered_app.rs) for a complete runnable version of this pattern.
...
- deep nesting through automatic `Default` chains across `Depends` structs
- explicit non-dependency fields via `#[arg]` on `Depends` structs
